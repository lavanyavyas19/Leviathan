...
const BottomChart = ({ onChartClick, onAnomalyFilter, onVesselHighlight }) => {
  ...
  const peakValues = useMemo(() => {
    ...
    return {
      spoofing: maxSpoofing.index >= 0 ? { ...chartData[maxSpoofing.index], index: maxSpoofing.index } : null,
      loitering: maxLoitering.index >= 0 ? { ...chartData[maxLoitering.index], index: maxLoitering.index } : null
    };
  }, [chartData]);
  ...
  const spikeAnnotations = useMemo(() => {
    ...
    const threshold = avgTotal * 1.5; // 50% above baseline
  ...
  const insightsSummary = useMemo(() => {
    ...
    if (maxTotal > avgTotal * 1.5) {
      insights.push(`Peak anomaly activity ${Math.round((maxTotal / avgTotal - 1) * 100)}% above baseline`);
    }
    
    if (totalSpoofing > totalLoitering) {
      insights.push(`Spoofing incidents dominate (${totalSpoofing} vs ${totalLoitering} loitering)`);
    ...
    const trend = chartData.length > 1 
      ? (chartData[chartData.length - 1].total > chartData[0].total ? 'increasing' : 'decreasing')
      : 'stable';
    insights.push(`Overall trend: ${trend}`);
    
    return insights.length > 0 ? insights.join('. ') + '.' : 'No significant anomalies detected in this period.';
  ...
  const displayData = useMemo(() => {
    ...
    return chartData.map(d => ({
      ...d,
      spoofing: vesselCount > 0 ? ((d.spoofing / vesselCount) * 100).toFixed(2) : d.spoofing,
      loitering: vesselCount > 0 ? ((d.loitering / vesselCount) * 100).toFixed(2) : d.loitering,
      total: vesselCount > 0 ? ((d.total / vesselCount) * 100).toFixed(2) : d.total,
      other: vesselCount > 0 ? ((d.other / vesselCount) * 100).toFixed(2) : d.other
    }));
  }, [chartData, normalized, vesselCount]);

  // Get aggregation subtitle
  ...
  const handleChartClick = (data) => {
    if (data && data.activePayload && data.activePayload[0]) {
      ...
      // Determine anomaly type from clicked line
      let anomalyType = null;
      if (clickedDataKey === 'spoofing') {
        anomalyType = 'spoofing';
      } else if (clickedDataKey === 'loitering') {
        anomalyType = 'loitering';
      }
      
      // Filter vessels and highlight on map
      ...
      // Highlight vessels on map (mock - would need actual vessel IDs)
      if (onVesselHighlight && clickedData) {
        // In real implementation, this would filter vessels by timestamp and anomaly type
      ...
      // Navigate to dashboard if callback exists
      if (onChartClick) {
  ...
  const PeakDot = (props) => {
    const { cx, cy, payload, dataKey, peakValue } = props;
    if (!peakValue || peakValue.index !== payload.index) return null;
      ...
      <div className="flex items-center justify-center space-x-4 mb-3 px-2">
        <div className="flex items-center space-x-2 px-3 py-1.5 bg-slate-800/50 rounded-lg border border-red-500/30">
          ...
          <span className="text-sm font-semibold text-red-400">
            {normalized 
              ? `${((summaryTotals.spoofing / vesselCount) * 100).toFixed(2)}%`
              : summaryTotals.spoofing
            }
          </span>
        </div>
        <div className="flex items-center space-x-2 px-3 py-1.5 bg-slate-800/50 rounded-lg border border-orange-500/30">
          ...
          <span className="text-sm font-semibold text-orange-400">
            {normalized 
              ? `${((summaryTotals.loitering / vesselCount) * 100).toFixed(2)}%`
              : summaryTotals.loitering
            }
          </span>
        </div>
        <div className="flex items-center space-x-2 px-3 py-1.5 bg-slate-800/50 rounded-lg border border-cyan-500/30">
          ...
          <span className="text-sm font-semibold text-cyan-400">
            {normalized 
              ? `${((summaryTotals.total / vesselCount) * 100).toFixed(2)}%`
              : summaryTotals.total
            }
          </span>
        </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <YAxis 
              ...
              tickLine={false}
              tick={{ fill: '#94a3b8', fontSize: 12 }}
              label={normalized ? { value: 'Per 100 vessels', angle: -90, position: 'insideLeft', fill: '#94a3b8', fontSize: 10 } : undefined}
            />
            ...
            {showBaseline && (
              <ReferenceLine 
                y={normalized ? ((baselineValue / vesselCount) * 100).toFixed(2) : baselineValue} 
                stroke="#64748b" 
                strokeWidth={1.5}
                strokeDasharray="3 3"
                opacity={0.4}
            ...
            {/* Spike Annotations - Note: Annotations are shown in the insights summary below */}
            ...
            <Line
              ...
              stroke="#ff4444"
              strokeWidth={2}
              dot={(props) => <PeakDot {...props} peakValue={peakValues.spoofing} dataKey="spoofing" />}
            ...
            <Line
              ...
              stroke="#ff8c00"
              strokeWidth={2}
              dot={(props) => <PeakDot {...props} peakValue={peakValues.loitering} dataKey="loitering" />}
